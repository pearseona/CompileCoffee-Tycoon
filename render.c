...
void render_frame(SDL_Renderer* ren, Game* g) {
	...
	if (g->state == STATE_PLAYING || g->state == STATE_CLOSING) {
		...
		char uiBuf[256];
		// 🛠️ 여기에 sizeof(uiBuf)를 완벽하게 추가했습니다!
		...
		sprintf_s(shopBuf, sizeof(shopBuf), "♥ 제 %d 일 차  영 영업준비단계 (상점) ♥", g->day);
		...
		sprintf_s(shopBuf, sizeof(shopBuf), "[8]번 키 : 제조 슬롯 확장 [비용: 3,000원] (%d/%d)", g->upg[0].level, g->upg[0].max_level);
		draw_text(ren, g_fnt_sm, shopBuf, 90, 270, light_gray);

		sprintf_s(shopBuf, sizeof(shopBuf), "[9]번 키 : 고속 커피 머신 도입 [비용: 25,000원] (%d/%d)", g->upg[1].level, g->upg[1].max_level);
		draw_text(ren, g_fnt_sm, shopBuf, 90, 310, light_gray);

		draw_text(ren, g_fnt_md, "➔ 정비를 마쳤다면 [Enter]를 눌러 영업을 시작합니다.", 70, SCREEN_H - 140, gold);
	}
